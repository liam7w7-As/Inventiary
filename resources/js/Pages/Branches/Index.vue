<template>
    <AppLayout>
        <template #title>Sucursales</template>
        ...
        <div class="branches-page">
            ...
            <div class="page-header">
                <div>
                    ...
                    <p class="page-desc">Administra las sucursales de tu negocio</p>
                </div>
                <a href="/branches/create" @click.prevent="$inertia.visit('/branches/create')" class="btn-primary">
            ...
            <!-- Table -->
            <AppTable :columns="columns" :rows="branches.data">
                ...
                <template #cell(manager)="{ row }">
                    ...
                    <AppBadge v-else variant="warning">Sin encargado</AppBadge>
                </template>
                ...
                <template #cell(actions)="{ row }">
                    <div class="actions-group">
                        <a :href="`/branches/${row.id}/edit`" @click.prevent="$inertia.visit(`/branches/${row.id}/edit`)" class="action-btn action-edit" title="Editar">
                        ...
                        <button class="action-btn action-toggle" :title="row.is_active ? 'Desactivar' : 'Activar'" @click="toggleBranch(row)">
            ...
            <!-- Pagination -->
            <div v-if="branches.links && branches.last_page > 1" class="pagination">
        ...
        <ConfirmModal
            ...
            :message="`¿Estás seguro de eliminar la sucursal '${deleteModal.branch?.name}'? Asegúrate de que no tenga usuarios, productos ni ventas asociadas.`"
...
function clearFilters() {
    filters.search = ''
    filters.is_active = ''
    router.get('/branches', {}, { preserveState: true, replace: true })
}
...
const columns = [
    ...
    { key: 'manager', label: 'Encargado asignado' },
    { key: 'is_active', label: 'Estado' },
    { key: 'actions', label: 'Acciones', width: '120px' },
]

function toggleBranch(branch) {
    router.patch(`/branches/${branch.id}/toggle-status`, {}, { preserveState: true })
}

const deleteModal = reactive({ show: false, branch: null })
...
function executeDelete() {
    router.delete(`/branches/${deleteModal.branch.id}`, {
        preserveState: true,
